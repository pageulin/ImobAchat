...
;(function($){
	...
	function search_initialize(){		//alert('init');
		...
		if(mapSearchBox.length && mapBox.length){
			...
			setTimeout(function() {
				...
				google.maps.event.addListener(searchBox, 'places_changed', function() {				//alert('places changed');
					...
					$("#noo_property_gmap_longitude").val(longitude);
					$("#noo_property_gmap_latitude").val(latitude);
	
					$('#load_section').load('includes/setLongLatSession.php?longitude='+longitude+'&latitude='+latitude);
			...
			var infoboxOptions = {
                    ...
                    zIndex: null,                    closeBoxMargin: "-13px 0px 0px 0px",
			 ...
			 if(markers.length){
				 ...
				 for(var i = 0; i < markers.length ; i ++){
					...
					google.maps.event.addListener(gmarker, 'click', function(e) {
						...
						var infoContent = '<div class="gmap-infobox"><label class="btn-close" onclick="return infoBox.close();"><i class="fa fa-close"></i></label>';

						infoContent += '<div class="info-img">' + this.image + '</div>';

						infoContent += '<div class="info-summary">';

						infoContent += '<h5>' + this.title + '</h5>';

						infoContent += '<div class="info-detail">';

						infoContent += '<div class="size"><span>' + this.area + '</span></div>';

						infoContent += '<div class="bedrooms"><span>' + this.bedrooms + '</span></div>';

						infoContent += '<div class="bathrooms"><span>' + this.bathrooms + '</span></div>';

						infoContent += '</div>';

						infoContent += '<div class="info-more">';

						infoContent += '<div class="info-price">' + this.price_html + '</div>';

						infoContent += '<div class="info-action"><a href="' + this.url + '"><i class="fa fa-plus"></i></a></div>';
			...
			var clusterStyles = [{
					...
					url:'resources/images/icon/cloud.png',
	...
	function mylocationCallback(pos){
		  ...
		  var shape = {
		   ...
		   var marker = new google.maps.Marker({
	             ...
	             icon: 'resources/images/icon/my-marker.png',
	...
	function single_initialize(){
		...
		if(mapBox.length){
			...
			var marker = new google.maps.Marker({
				...
				icon:  'resources/images/icon/marker-icon.png',
